...
- **Node.js**: `>=22.0.0`
- **pnpm**: `>=9.0.0`
- **Docker & Docker Compose**: (Required for containerized runtime and local deterministic PostgreSQL cache stack setup)
...
# 2. Boot the backing services (Deterministic PostgreSQL Cache)
...
You can run the complete stack (Deterministic PostgreSQL Cache, Indexer, and API) using a single command:
...
- **Deterministic PostgreSQL Cache**: Exposes a native `pg_isready` check.
- **Indexer & API**: Depend on the `postgres` healthcheck condition (`service_healthy`). Docker Compose ensures that the deterministic PostgreSQL cache is fully ready to accept connections before booting Node.js application containers.
...
| `DATABASE_URL` | Deterministic PostgreSQL Cache connection URL | `postgresql://postgres:postgres@localhost:5432/sera_data` |
...
3. **Close connections**: Deterministic PostgreSQL cache client pools are cleanly destroyed.
...
- **Deterministic PostgreSQL Cache Connectivity Failures**: If the API server returns `503 Service Unavailable` on `/health`, verify that the deterministic PostgreSQL cache is running and responsive, and confirm that the `DATABASE_URL` contains the correct host credentials.
